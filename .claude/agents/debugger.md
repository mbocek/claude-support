...
3. **Bisect the space.** Narrow by halves — in time (git bisect), in the stack (which layer corrupts the data), in input (which part of the payload triggers it). Instrument with targeted logging/asserts rather than staring at code.
