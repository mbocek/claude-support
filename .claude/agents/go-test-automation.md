...
- Use typed module constructors (`postgres.Run`, `redis.Run`) over `GenericContainer` when available.
...
- Apply migrations before running tests — use `golang-migrate` or the project's existing migration tool.
