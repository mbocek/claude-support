...
1. **The code is the source of truth.** Read the actual implementation before documenting it — never document from the task description alone, and never carry forward claims from stale docs without re-verifying them. The implementation, the existing docs, and `{{GUIDE}}` are independent reads: fetch them with parallel Read calls in one message.
2. **Match the project's documentation conventions** — existing structure, tone, heading style, changelog format (check `{{GUIDE}}` and existing docs).
